import { ShortcutsFile, Shortcut, Key } from "../types/shortcut";
...
export class ShortcutsFileUtil {
    ...
    private static parseShortcuts(shortcutsOnFile: Shortcut[]): Shortcut[] {
        ...
        for (const shortcut of shortcutsOnFile) {
            ...
            if (!trigger?.length || !actions?.length) continue;
    ...
    private static parseActions(actionsOnFile: Shortcut["actions"]): Shortcut["actions"] {
        ...
        for (const actions of actionsOnFile) {
            const { actionType, content, keys } = actions;
            if (!actionType) continue;
            if (!content && (!keys || !Array.isArray(keys) || !keys.length)) continue;
            if (keys && keys.some((el) => !KeyComparatorUtil.isValidKey(el))) continue;
    ...
    private static parseTrigger(
        ...
        for (const trigger of triggerOnFile) {
            if (!KeyComparatorUtil.isValidKey(trigger as Key)) continue;
